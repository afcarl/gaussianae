...
class MDNSharedParams(DenseLayer):
    ...
    def __init__(self, incoming, mus=None, sigmas=None, corxy=None, **kwargs):
        ...
        if sigmas is not None:
            ...
            #sigmas_init = np.array([10, 10]).astype('float32') * np.abs(np.random.randn(self.num_units, 2).reshape((self.num_units,2))).astype('float32')
            sigmas_init = np.random.uniform(low=5, high=15, size=(self.num_units, 2)).astype('float32')
    ...
    def get_output_for(self, input, **kwargs):
        ...
        activation = T.dot(input, self.W)
        if self.b is not None:
            activation = activation + self.b.dimshuffle('x', 0) + 1e-6
        return self.nonlinearity(activation)

class BivariateGaussianLayerWithPi(Layer):

    def __init__(self, incoming, num_units, mus=None, sigmas=None, corxy=None, **kwargs):
        super(BivariateGaussianLayerWithPi, self).__init__(incoming, **kwargs)
        #self.H = H
        #self.H = self.add_param(H, (H.shape[0], H.shape[1]), name='H')
        self.name = 'BivariateGaussianLayerWithPi'
        self.num_units = num_units
        if mus is not None:
            mus_init = mus
        else:
    ...
    def get_output_for(self, input, **kwargs):
        ...
        #probs = (0.5 / np.pi) * sigmainvprods * T.sqrt(oneminuscorxy2inv) * T.exp(expterm)
        #probs = T.exp(T.log(0.5/np.pi) + T.log(sigmainvprods) + T.log(T.sqrt(oneminuscorxy2inv)) + expterm)
        pis_activation = T.dot(input, self.W) + self.b.dimshuffle('x', 0)
        pis = T.nnet.softmax(pis_activation)
        self.pis = pis
        probs = T.exp(T.log(0.5/np.pi) + T.log(sigmainvprods) + T.log(T.sqrt(oneminuscorxy2inv)) + expterm + T.log(pis))
        return probs   
